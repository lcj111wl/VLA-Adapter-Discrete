...
def run_forward_pass(
    ...
    if cfg.use_discrete_head:
        # Get last layer hidden states (Same as L1 Regression)
        ...
        action_logits = action_head.module.predict_action(
            multi_layer_hidden_states,
            proprio=batch["proprio"] if use_proprio else None,
            proprio_projector=proprio_projector if use_proprio else None,
            phase=cfg.phase,
        ...
        # Shape: (Batch, Chunk, Action_Dim)
        ground_truth_indices = vla.module.discretize_actions(ground_truth_actions)

        # Compute Cross Entropy Loss
        # Flatten for CE Loss: (N, C) vs (N)
        # N = Batch * Chunk * Action_Dim
        # C = N_Bins
        loss = torch.nn.CrossEntropyLoss()(
            action_logits.reshape(-1, cfg.n_action_bins), 
            ground_truth_indices.reshape(-1)
        )

        # Compute Accuracy
        with torch.no_grad():
            # Argmax to get predicted indices
            predicted_indices = action_logits.argmax(dim=-1)
            accuracy = (predicted_indices == ground_truth_indices).float().mean()
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    # If applicable, instantiate discrete action head
    elif cfg.use_discrete_head:
        action_head = init_module(
        DiscreteActionHead,
        "action_head",
        cfg,
